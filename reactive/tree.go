...
func (b *DataTree) Track(attr string) (Observers, error) {
	b.rw.RLock()
...
func (b *DataTree) registerObserver(name string, ob Observers) {
	var ok bool
	b.rw.RLock()
...
func RegisterFieldsObservers(tree DataTreeRegister, treeable interface{}) error {
	...
	for i := 0; i < rot.NumField(); i++ {
		...
		rcfl := fl.Elem().Interface().(Observers)
...
func RegisterReflectWith(tree DataTreeRegister, name string, rot reflect.Value) error {
	...
	rcfl := rot.Elem().Interface().(Observers)
